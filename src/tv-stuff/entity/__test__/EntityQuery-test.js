...
const EntityQueryModule = proxyquire('../EntityQuery', {
    'tv-stuff/util/request': function(url, options) {
...
process.env.TV_STUFF_GRAPHQL_SERVER = 'http://localhost:3000';
...
test('EntityQuery Throws error if TV_STUFF_GRAPHQL_SERVER env var isn\'t set', tt => {
    process.env.TV_STUFF_GRAPHQL_SERVER = '';
    tt.throws(
        () => requestActionThunk(()=>{}),
